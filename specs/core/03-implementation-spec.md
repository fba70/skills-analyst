...
**Reviewers:** TBD · **Product spec:** `02-requirements-spec.md` · **Business concept:** `01-business-concept.md`

## Summary

We build Skill Foundry v1 on Next.js/Vercel (Node runtime, front + back), Drizzle ORM + Neon Postgres (`pg` over TCP to the pooled endpoint — a deliberate departure from the house HTTP-driver pattern, because this system's multi-write invariants need transactions), better-auth for accounts/orgs/API keys, and Cloudflare R2 for skill content with content-addressed keys. All long-running work (source sync, validation, archetype mining) runs as durable workflows fanned out one step per skill, dispatched by cron from database-held schedules. LLM analyzers and the assistant go exclusively through the Vercel AI Gateway with runtime-resolved model IDs and hard spend metering. The single most important consequence of this design: search, similarity, and analytics all stay inside Postgres (FTS + pgvector + read replica), so v1 ships with exactly four vendors and no self-managed infrastructure.
...
No sandboxed execution of third-party skill code in v1 — the v1 trust boundary is "no ingested code executes anywhere"; the R2.10 sandbox is its own spec (Phase 4, likely a separate egress-denied Worker/Container runner). No Kubernetes, no self-managed queues or search clusters. No BYO-model for users. No multi-region. Postgres RLS backstop is deferred to its own spec (see Constraints C4 and Alternatives).

## Constraints and assumptions

- **C1 (fixed):** Stack is Next.js on Vercel, Drizzle + Neon, better-auth, Cloudflare R2 — chosen for team familiarity and operational simplicity; this spec selects *within* it and justifies any departure from house defaults.
...
- **C4 (accepted debt):** Tenant isolation is DAL-enforced only in v1; RLS is deferred and this is stated in every data-access review until it lands.
...
**Job execution: cron dispatcher + durable workflows, fan-out one step per skill.** The dispatcher (Vercel Cron, 15-min tick) reads schedules from `sources` — cadence is data, not deploys — writes a heartbeat event, and launches sync workflows. Idempotency key for a sync step is `(source_id, path, commit_sha)`; at-least-once redelivery is safe by construction. Validation workflows chain per skill version: cheap rule-based analyzers and secret scanning in-step, then LLM analyzers. Do not collapse the fan-out: per-item steps are what make hour-scale corpus jobs survivable and resumable. Cloudflare Queues were considered for this and rejected for v1 (see Alternatives).

**LLM access: Vercel AI Gateway only, no raw provider SDKs.** Model IDs resolve at runtime from a config table (analyzer model swap = config change, and every verdict records analyzer + model version, keeping re-scans targeted). Analyzer prompts treat skill content as data (C3): hardened prompt structure, structured output with schema validation, and *a schema-validation failure is itself a quarantine reason* — an analyzer that got prompt-injected into free-text is detected by its own output shape. The assistant is an AI SDK agent loop with explicit `stopWhen` bounds and a closed tool surface (retrieve archetype, retrieve exemplars, similarity report, lint draft). Gateway budgets are soft caps checked at request start, so the hard stop is our own per-org metering counter (RC.2), checked before each call.
...
Shared tables + `org_id`, enforced in the DAL where every query resolves the org from the better-auth session. Public-corpus reads (`org_id IS NULL`) are the safe common case; risk concentrates in Team-tier private corpora, which is why entitlements (RC.1) and isolation live in the same DAL layer and ship together. **The database backstop (RLS) does not exist in v1** — stated per house rule, revisited as its own spec before the first enterprise tenant with a private corpus goes live. That sequencing is deliberate: RLS lands when the data it protects starts existing.
...
|---|---|---|---|
| 1 | Durable workflow engine: Vercel Workflows vs Inngest-class — decide on per-step observability, retry semantics, and local dev story (spike, 2 days) | Eng | Before sync build |
...
| 6 | RLS spec scheduling — hard trigger: first Team-tier private-corpus tenant signed | Eng | Tier launch |
